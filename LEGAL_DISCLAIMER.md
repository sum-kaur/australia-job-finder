...
`australia-job-finder` is a collection of Markdown prompts, Node.js scripts, and HTML templates. It is strictly a local execution tool. The maintainers do not host, deploy, or operate an AI system, nor do we provide API access to Large Language Models (LLMs).
...
australia-job-finder interacts with career portals and job boards (Greenhouse, Lever, Ashby, Workday, and others).
...
australia-job-finder is designed to help individuals make better career decisions, not to automate away human judgment. Acceptable use includes:
...
The "australia-job-finder" name and brand identity are separate from the MIT
